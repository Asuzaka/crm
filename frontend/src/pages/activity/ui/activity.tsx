...
export function Activity(){
  ...
  const {data, isPending, error} = useGetRecords(1, 20);


  // helper 
  function returnColorOfAction(a: action): string {
    switch (a) {
      case "LOGIN":
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data?.data.map((activity) => (
                <tr key={activity._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${returnColorOfAction(activity.actionType)}`}
                    >
