...
import { ArrowLeftIcon, UserIcon, TagIcon, CreditCardIcon, DollarSignIcon, FileTextIcon } from "lucide-react";
import { getColorOfCategory } from "../../expenses/helper/get-color";
import { Loader } from "@/shared/ui";
import { useGetExpense } from "@/features/expense";
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg">
        <div className="px-4 py-5 sm:px-6 bg-gray-50">
          <h2 className="text-lg leading-6 font-medium text-gray-900">{data.data.description}</h2>
          <p className="mt-1 max-w-2xl text-sm text-gray-500">
            Expense #{data.data._id} | Created on {new Date(data.data.createdAt).toLocaleDateString()}
          </p>
        </div>
        <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
          <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
            <div className="sm:col-span-1">
              ...
              <dd className="mt-1 text-lg font-semibold text-red-600">{data.data.amount} UZS</dd>
            </div>
            {/* <div className="sm:col-span-1">
              <dt className="text-sm font-medium text-gray-500 flex items-center">
            ...
            <div className="sm:col-span-1">
              ...
              <dd className="mt-1 text-sm text-gray-900">
                {data.data.recipientType === "Manager/Staff" && data.data.manager?.name}
                {data.data.recipientType === "External Vendor" && data.data.vendorName}
                {data.data.manager && <span className="text-xs text-gray-500 ml-2">(Staff)</span>}
                {!data.data.manager && <span className="text-xs text-gray-500 ml-2">(External)</span>}
              </dd>
            </div>
            <div className="sm:col-span-1">
              <dt className="text-sm font-medium text-gray-500 flex items-center">
              ...
              <dd className="mt-1 text-sm text-gray-900">{data.data.paymentMethod}</dd>
            </div>
            {/* <div className="sm:col-span-1">
              <dt className="text-sm font-medium text-gray-500 flex items-center">
                <UserIcon className="h-5 w-5 mr-2 text-gray-400" />
