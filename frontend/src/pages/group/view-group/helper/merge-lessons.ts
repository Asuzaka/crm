...
export function mergeLessons(existing: LessonRecord[], generated: LessonRecord[]): LessonRecord[] {
  ...
  return generated.map((g) => {
    ...
    const mergedStudents = g.students.map((gs) => {
      const existingStudent = existingLesson.students.find((es) => es.student === gs.student);
