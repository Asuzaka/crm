...
import { INVALIDID, NODOCUMENTFOUND, NOIDPROVIDED, SUCCESS } from "../constants/errors";
...
// get Records
export const getRecords = catchAsync(async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  const featuresForCount = new apiFeatures(query, req.query).filter().search(["description"]);
...
// get Record // id
export const getRecord = catchAsync(async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const { id } = req.params;
  ...
  const record: IRecord | null = await Record.findById(id).populate("user", "_id name");
...
export const createRecords = catchAsync(async (_req: AuthenticatedRequest, _res: Response, _next: NextFunction) => {});
// patch Record update //id
export const updateRecords = catchAsync(async (_req: AuthenticatedRequest, _res: Response, _next: NextFunction) => {});
// delete Record delete
export const deleteRecords = catchAsync(async (_req: AuthenticatedRequest, _res: Response, _next: NextFunction) => {});
